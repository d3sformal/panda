...
class Lift extends Thread {
    ...
    private void doMoving() {
        ...
        //      floor
        if (((pickupOn[currentFloor] == UP) || (pickupOn[currentFloor] == UP+DOWN)) ||
        ...
        //      floor
        if (((pickupOn[currentFloor] == DOWN) || (pickupOn[currentFloor] == DOWN+UP)) ||
    ...
    private void addPeople(Vector<Integer> people) {
        ...
        for (Iterator<Integer> it = people.iterator(); it.hasNext(); ) {
            int toFloor = it.next().intValue();
            peopleFor[toFloor] += 1;
        }
    }
}
