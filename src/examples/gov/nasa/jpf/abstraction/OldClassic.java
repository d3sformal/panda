 ...
 * ('FirstTask', 'SecondTask'), without synchronization with the corresponding
 * monitor operations ('wait_for-Event()' and 'signalEvent()').
 * 
 * The resulting race is typical for unsafe optimizations that try to avoid
 * expensive blocking calls by means of local caches
 * 
 * This example was inspired by a defect found in the "Remote Agent" spacecraft
 * controller that flew on board of "Deep Space 1", as described in:
 * 
...
class Event {
	int count = Debug.makeAbstractInteger(0);
	...
	public synchronized void signal_event() {
		...
		// count = (count + 1) % 3;
		count = Debug.makeAbstractInteger(count);
		count++;
...
class FirstTask extends Thread {
	Event event1;
	Event event2;
	int count = Debug.makeAbstractInteger(0); // bad optimization - local cache
												// of event1 internals
	...
	public void run() {
		count = Debug.makeAbstractInteger(event1.count); // <race> violates
															// event1 monitor
															// encapsulation
		...
		while (true) {
			...
			count = Debug.makeAbstractInteger(event1.count); // <race> ditto
...
class SecondTask extends Thread {
	...
	public void run() {
		count = Debug.makeAbstractInteger(event2.count); // <race> violates
															// event2 monitor
															// encapsulation
		...
		while (true) {
			...
			count = Debug.makeAbstractInteger(event2.count); // <race> ditto
		}
	}
}
