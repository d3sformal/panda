...
final public class ReplicatedWorkers {
	...
	public ReplicatedWorkers(Configuration newConfig,
			Collection workCollection, Collection resultCollection,
		...
		workers = new Vector<Object>(numWorkers);
...
final class Coordinator {
	...
	protected final synchronized void mainAwaitTerminate() { // seeded error:
																// change != to
																// ==
		while (blockedTakers == memberOf.numWorkers && !memberOf.done) {
			try {
				wait();
	...
	public void run() {
		...
		while (true) {
			...
			while (true) {
				...
				// done = false;
				done = Verify.randomBool(); // external non-determinism
