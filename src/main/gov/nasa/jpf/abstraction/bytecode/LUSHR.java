...
public class LUSHR extends gov.nasa.jpf.jvm.bytecode.LUSHR {
	...
	public Instruction execute(ThreadInfo ti) {
		...
		if (abs_v1 == null && abs_v2 == null)
			...
			// abs_v2 >>> abs_v1
			Abstraction result = Abstraction._ushr(v1, abs_v1, v2, abs_v2);
			System.out.printf("LUSHR> Values: %d (%s), %d (%s)\n", v2, abs_v2, v1, abs_v1);
			...
			if (result.isComposite()) {
				...
				if (!ti.isFirstStepInsn()) { // first time around
					...
					System.out.printf("LUSHR> Result: %s\n", result);
				}
			} else
				System.out.printf("LUSR> Result: %s\n", result);
