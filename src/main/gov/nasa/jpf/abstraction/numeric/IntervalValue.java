...
public class IntervalValue extends AbstractValue {
	...
	public AbstractValue _plus(int right) {
		if (right == 1) {
			...
			boolean in = (can_be_INSIDE() && ((IntervalAbstraction)abs).MIN != ((IntervalAbstraction)abs).MAX) || can_be_LESS();
			boolean le = can_be_LESS();
			return ((IntervalAbstraction)abs).create(le, in, gr);
		} else if (right == -1) {
			boolean le = can_be_INSIDE() || can_be_LESS();
			boolean in = (can_be_INSIDE() && ((IntervalAbstraction)abs).MIN != ((IntervalAbstraction)abs).MAX)
					|| can_be_GREATER();
			boolean gr = can_be_GREATER();
			return ((IntervalAbstraction)abs).create(le, in, gr);
		} else
			return _plus(((IntervalAbstraction)abs).abstractMap(right));
	}
	...
	public AbstractValue _plus(AbstractValue right) {
		if (right instanceof IntervalValue) {
			...
			if (this.can_be_LESS()) {
				...
				right1 = Math.max(right1, ((IntervalAbstraction)abs).MIN);
			}
			if (this.can_be_INSIDE()) {
				left1 = Math.min(left1, ((IntervalAbstraction)abs).MIN);
				right1 = Math.max(right1, ((IntervalAbstraction)abs).MAX);
			}
			if (this.can_be_GREATER()) {
				left1 = Math.min(left1, ((IntervalAbstraction)abs).MAX);
				right1 = Math.max(right1, Double.POSITIVE_INFINITY);
			}
			if (right_value.can_be_LESS()) {
				left2 = Math.min(left2, Double.NEGATIVE_INFINITY);
				right2 = Math.max(right2, ((IntervalAbstraction)abs).MIN);
			}
			if (right_value.can_be_INSIDE()) {
				left2 = Math.min(left2, ((IntervalAbstraction)abs).MIN);
				right2 = Math.max(right2, ((IntervalAbstraction)abs).MAX);
			}
			if (right_value.can_be_GREATER()) {
				left2 = Math.min(left2, ((IntervalAbstraction)abs).MAX);
				right2 = Math.max(right2, Double.POSITIVE_INFINITY);
			}
			double res_left = left1 + left2, res_right = right1 + right2;
			return ((IntervalAbstraction)abs).create(res_left < ((IntervalAbstraction)abs).MIN, res_right >= ((IntervalAbstraction)abs).MIN
					&& res_left <= ((IntervalAbstraction)abs).MAX, res_right > ((IntervalAbstraction)abs).MAX);
		} else
	...
	public AbstractValue _minus(int right) {
		return _minus(((IntervalAbstraction)abs).abstractMap(right));
	}
	...
	public AbstractValue _mul(int right) {
		return _mul(((IntervalAbstraction)abs).abstractMap(right));
	}
	...
	public AbstractValue _div(int right) {
		return _div(((IntervalAbstraction)abs).abstractMap(right));
	}
	...
	public AbstractValue _rem(int right) {
		return _rem(((IntervalAbstraction)abs).abstractMap(right));
	}
	...
	public AbstractValue _bitwise_and(int right) {
		return _bitwise_and(((IntervalAbstraction)abs).abstractMap(right));
	}
	...
	public AbstractValue _bitwise_and(AbstractValue right) {
		if (right instanceof IntervalValue) {
			return ((IntervalAbstraction)abs).create(true, true, true);
		} else
			throw new RuntimeException("## Error: unknown abstraction");
	}
	...
	public AbstractValue _bitwise_or(int right) {
		return _bitwise_or(((IntervalAbstraction)abs).abstractMap(right));
	}
	...
	public AbstractValue _bitwise_xor(int right) {
		return _bitwise_xor(((IntervalAbstraction)abs).abstractMap(right));
	}
	...
	public AbstractValue _shift_left(int right) {
		return _shift_left(((IntervalAbstraction)abs).abstractMap(right));
	}
	...
	public AbstractValue _shift_right(int right) {
		return _shift_right(((IntervalAbstraction)abs).abstractMap(right));
	}
	...
	public AbstractValue _unsigned_shift_right(int right) {
		return _unsigned_shift_right(((IntervalAbstraction)abs).abstractMap(right));
	}
	...
	public AbstractValue _neg() {
		...
		if (this.can_be_LESS()) {
			...
			right = Math.max(right, ((IntervalAbstraction)abs).MIN);
		}
		if (this.can_be_INSIDE()) {
			left = Math.min(left, ((IntervalAbstraction)abs).MIN);
			right = Math.max(right, ((IntervalAbstraction)abs).MAX);
		}
		if (this.can_be_GREATER()) {
			left = Math.min(left, ((IntervalAbstraction)abs).MAX);
	...
	public AbstractBoolean _ge(int right) {
		return _ge(((IntervalAbstraction)abs).abstractMap(right));
	}
	...
	public AbstractBoolean _gt(int right) {
		return _gt(((IntervalAbstraction)abs).abstractMap(right));
	}
	...
	public AbstractBoolean _le(int right) {
		return _le(((IntervalAbstraction)abs).abstractMap(right));
	}
	...
	public AbstractBoolean _lt(int right) {
		return _lt(((IntervalAbstraction)abs).abstractMap(right));
	}
	...
	public AbstractBoolean _eq(int right) {
		return _eq(((IntervalAbstraction)abs).abstractMap(right));
	}
	...
	public AbstractBoolean _ne(int right) {
		return _ne(((IntervalAbstraction)abs).abstractMap(right));
	}
	...
	public AbstractValue _cmp(long right) {
		return this._cmp(((IntervalAbstraction)abs).abstractMap(right));
	}
	...
	public AbstractValue _cmpg(float right) {
		return this._cmpg(((IntervalAbstraction)abs).abstractMap(right));
	}
	...
	public AbstractValue _cmpl(float right) {
		return this._cmpl(((IntervalAbstraction)abs).abstractMap(right));
	}
	...
	protected AbstractValue _div_reverse(int right) {
		return ((IntervalAbstraction)abs).abstractMap(right)._div(this);
	}
	...
	protected AbstractValue _cmp_reverse(long right) {
		return ((IntervalAbstraction)abs).abstractMap(right)._cmp(this);
	}

	@Override
	protected AbstractValue _cmpl_reverse(float right) {
		return ((IntervalAbstraction)abs).abstractMap(right)._cmpl(this);
	}
	...
	protected AbstractValue _cmpg_reverse(float right) {
		return ((IntervalAbstraction)abs).abstractMap(right)._cmpg(this);
	}
	...
	protected AbstractValue _rem_reverse(int right) {
		return ((IntervalAbstraction)abs).abstractMap(right)._rem(this);
	}
	...
	protected AbstractValue _shift_left_reverse(int right) {
		return ((IntervalAbstraction)abs).abstractMap(right)._shift_left(this);
	}
	...
	protected AbstractValue _shift_right_reverse(int right) {
		return ((IntervalAbstraction)abs).abstractMap(right)._shift_right(this);
	}
	...
	protected AbstractValue _unsigned_shift_right_reverse(int right) {
		return ((IntervalAbstraction)abs).abstractMap(right)._unsigned_shift_right(this);
	}
	...
	protected AbstractBoolean _lt_reverse(int right) {
		return ((IntervalAbstraction)abs).abstractMap(right)._lt(this);
	}

	@Override
	protected AbstractBoolean _le_reverse(int right) {
		return ((IntervalAbstraction)abs).abstractMap(right)._le(this);
	}

	@Override
	protected AbstractBoolean _ge_reverse(int right) {
		return ((IntervalAbstraction)abs).abstractMap(right)._ge(this);
	}

	@Override
	protected AbstractBoolean _gt_reverse(int right) {
		return ((IntervalAbstraction)abs).abstractMap(right)._gt(this);
	}
