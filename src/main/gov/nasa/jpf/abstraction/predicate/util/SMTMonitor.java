...
public class SMTMonitor extends SMTListener {
	...
	public void valuatePredicatesInvoked(Map<Predicate, PredicateDeterminant> predicates) {
		...
		for (Predicate p : predicates.keySet()) {
			System.out.println("\t" + p.toString(AccessExpression.NotationPolicy.DOT_NOTATION) + " WP(+): " + predicates.get(p).positiveWeakestPrecondition.toString(AccessExpression.NotationPolicy.DOT_NOTATION) + " WP(-): " + predicates.get(p).negativeWeakestPrecondition.toString(AccessExpression.NotationPolicy.DOT_NOTATION));
			
			Map<Predicate, TruthValue> determinants = predicates.get(p).determinants;
			
			System.out.println("\tDET:");
			for (Predicate d : determinants.keySet()) {
				System.out.println("\t\t" + d.toString(AccessExpression.NotationPolicy.DOT_NOTATION) + " " + determinants.get(d));
			}
		}
	...
	public void valuatePredicatesInvoked(Set<Predicate> predicates) {
		...
		for (Predicate p : predicates) {
			System.out.println("\t" + p.toString(AccessExpression.NotationPolicy.DOT_NOTATION));
		}
	...
	public void valuatePredicatesExecuted(Map<Predicate, TruthValue> valuation) {
		...
		for (Predicate p : valuation.keySet()) {
			System.out.println("\t" + p.toString(AccessExpression.NotationPolicy.DOT_NOTATION) + ": " + valuation.get(p));
		}
